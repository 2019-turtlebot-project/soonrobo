// Auto-generated. Do not edit!

// (in-package newmode.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;
let xyz = require('./xyz.js');

//-----------------------------------------------------------

class twist {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.linear = null;
      this.angular = null;
    }
    else {
      if (initObj.hasOwnProperty('linear')) {
        this.linear = initObj.linear
      }
      else {
        this.linear = new xyz();
      }
      if (initObj.hasOwnProperty('angular')) {
        this.angular = initObj.angular
      }
      else {
        this.angular = new xyz();
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type twist
    // Serialize message field [linear]
    bufferOffset = xyz.serialize(obj.linear, buffer, bufferOffset);
    // Serialize message field [angular]
    bufferOffset = xyz.serialize(obj.angular, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type twist
    let len;
    let data = new twist(null);
    // Deserialize message field [linear]
    data.linear = xyz.deserialize(buffer, bufferOffset);
    // Deserialize message field [angular]
    data.angular = xyz.deserialize(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 48;
  }

  static datatype() {
    // Returns string type for a message object
    return 'newmode/twist';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '9f195f881246fdfa2798d1d3eebca84a';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    xyz linear
    xyz angular
    
    ================================================================================
    MSG: newmode/xyz
    float64 x
    float64 y
    float64 z
    
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new twist(null);
    if (msg.linear !== undefined) {
      resolved.linear = xyz.Resolve(msg.linear)
    }
    else {
      resolved.linear = new xyz()
    }

    if (msg.angular !== undefined) {
      resolved.angular = xyz.Resolve(msg.angular)
    }
    else {
      resolved.angular = new xyz()
    }

    return resolved;
    }
};

module.exports = twist;
