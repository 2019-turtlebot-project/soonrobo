        	...
        	for cnt in contours:
            		...
            		if (self.area >= 500):
                		...
                		if self.red_count > 30:
                    			print(self.red_count)
            		elif (self.area >= 50) and (self.area < 500):
                		self.red_count = 0
                		#print(self.red_count)
            		if self.red_count == 30:
                		msg.mode = "track_bar"
...
def main():
	pub = rospy.Publisher('/image_raw2', Image, queue_size = 1)
        pub_det = rospy.Publisher('/det_msg',msg_detect, queue_size = 1)
	rospy.init_node("cam_tutorial")
	rate = rospy.Rate(10)
	play = Cap()
	play.cap = cv2.VideoCapture(1)
	play.cap.set(3, 320)
    	play.cap.set(4, 240)
    	if not play.cap.isOpened():
        	print("open fail camera")
	while not rospy.is_shutdown():
		play.ret, play.frame = play.cap.read()
		play.red_img()
		play.cont()
		k = cv2.waitKey(10)
